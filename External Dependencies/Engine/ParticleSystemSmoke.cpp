...
ParticleSystemSmoke::ParticleSystemSmoke()
{
...
void ParticleSystemSmoke::createShader()
{
	GLuint vertexShaderID = glCreateShader(GL_VERTEX_SHADER);
	GLuint fragShaderID = glCreateShader(GL_FRAGMENT_SHADER);
	programID = glCreateProgram();
	compileShader(vertexShaderID, "VertexShaderParticleSmoke.txt");
	compileShader(fragShaderID,"FragParticleShaderSmoke.txt");
...
void ParticleSystemSmoke::fillBuffer()
{
	glm::vec3 v(0.0f);
	...
	for( GLuint i = 0; i<particleCount; i++ ) 
	{
		...
		theta = glm::mix(0.0f, PI * 2.0f, randFloat());
		phi = glm::mix(0.0f, 2.0f * PI, randFloat());
...
void ParticleSystemSmoke::genVertexArrays()
{
		////////Work Around////////
	if(!drawBuf)
	{
...
void ParticleSystemSmoke::createFeedback()
{
...
void ParticleSystemSmoke::draw(const glm::mat4& projection,const glm::mat4& view,float dt)
{
	//Setup uniforms
	...
	glUniform1f(uniformLocation,4.0f);
	...
	uniformLocation = glGetUniformLocation(programID, "MaxParticleSize");
	glUniform1f(uniformLocation,20.0f);
	uniformLocation = glGetUniformLocation(programID, "MinParticleSize");
	glUniform1f(uniformLocation,10.0f);
